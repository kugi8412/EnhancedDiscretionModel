...
# Importujemy wszystkie augmentacje
from augment import (
    RandomDeletion, RandomInsertion, RandomTranslocation, 
    RandomInversion, RandomMutation, RandomRC, RandomNoise, NormalizeNoise # <--- DODANO NormalizeNoise
)
...
class DNADataset(Dataset):
    def __init__(self, X, Y_first, Y_second, augment=False, noise_config=None, evoaug_config=None, seed=None):
        ...
        # WAŻNE: Zawsze czytamy config EvoAug, aby poznać insert_max (nawet dla walidacji!)
        if evoaug_config:
    ...
    def _build_aug_list(self, aug_configs):
        aug_dict = {
            ...
            'NormalizeNoise': NormalizeNoise # <--- DODANO DO SŁOWNIKA
        }
    ...
    def __getitem__(self, idx):
        ...
        num_applied = 0
        inserted = False # Flaga sprawdzająca, czy sekwencja została sztucznie wydłużona insercją
        ...
        # --- ŻELAZNE WYRÓWNYWANIE WYMIARÓW ---
        # Jeśli insert_max jest większy od 0 (czyli używamy insercji w configu),
        # a dana sekwencja nie wylosowała mutacji insercji LUB jesteśmy w fazie walidacji:
        # MUSIMY wyrównać jej długość paddingiem!
