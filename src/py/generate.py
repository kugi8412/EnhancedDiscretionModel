#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Generate synthetic DNA enhancer sequences using a trained cVQ-VAE model."""
...
def onehot_to_string(onehot_tensor):
    """Convert a one-hot tensor [B, 4, L] to a list of DNA strings."""
...
def generate_random_dna(num_samples, seq_len=249, device='cuda'):
    """Generate purely random DNA baseline sequences."""
    indices = torch.randint(0, 4, (num_samples, seq_len)).to(device)
    return F.one_hot(indices, num_classes=4).transpose(1, 2).float()


def generate_from_vqvae(model, num_samples, dev_target, hk_target,
                        seq_len=249, device='cuda'):
    """Generate sequences de novo using VQ codebook and FiLM conditioning."""
    model.eval()
    
    with torch.no_grad():
        # Determine latent spatial dimensions via a dummy forward pass
        ...
        # FiLM conditioning on target expression
        ...
        # Take forward strand (channels 0-3) and argmax to hard one-hot
...
def main(args):
    ...
    print(f"[INFO] Initialising generation on device: {device}")
    
    # Load models
    config = load_config(args.config)
    
    print("[INFO] Building Generator (cVQ-VAE)...")
    vqvae = build_model(config).to(device)
    vqvae.load_state_dict(torch.load(args.weights, map_location=device, weights_only=True))
    vqvae.eval()
    
    print("[INFO] Building Oracle (LegNet)...")
    oracle_cfg = config.get('oracle', None)
    if not oracle_cfg or not oracle_cfg.get('apply', False):
        raise ValueError("YAML config must contain a valid 'oracle' section for evaluating generated sequences.")
    ...
    for group_name, params in groups.items():
        print(f"\n[INFO] Generating group: {group_name}...")
        ...
        with torch.no_grad():
            ...
            # Handle Oracle returning [B, 2] or tuple
    ...
    with open(fasta_path, "w") as f:
        f.write("\n".join(fasta_records))
    print(f"\n[SUCCESS] Saved {len(fasta_records)} sequences to {fasta_path}")
    ...
    ax.set_title('Inverse Design: Synthetic Enhancers Generated by cVQ-VAE', fontsize=14, pad=15)
    ...
    print(f"[SUCCESS] Prediction scatter plot saved to {plot_path}")
...
if __name__ == '__main__':
    ...
    # Target values (high / low expression)
    parser.add_argument('--high_dev', type=float, default=8.0, help="Target for High Dev")
    parser.add_argument('--low_dev', type=float, default=-2.0, help="Target for Low Dev")
    parser.add_argument('--high_hk', type=float, default=8.0, help="Target for High HK")
    parser.add_argument('--low_hk', type=float, default=-2.0, help="Target for Low HK")
