...
def decode_with_film(model, z_q, y_dev, y_hk, seq_len=249):
    """
    Ręcznie przeprowadza tensor ukryty przez warstwy FiLM i Dekoder.
    """
    ...
    return x_logits[:, 0:4, :] # Zwracamy tylko FWD (A,C,G,T)


# ==========================================
# 1. MODUŁ GRUPOWANIA PRZESTRZENI (QUADRANTS)
# ==========================================
...
class LatentNavigator(nn.Module):
    def __init__(self, num_embeddings, latent_len=62): 
        ...
        # Zerujemy wagi ostatniej warstwy
...
def main(args):
    ...
    print(f"[INFO] Inicjalizacja środowiska RL na urządzeniu: {device}")
    ...
    print(f"[INFO] Przestrzeń ukryta -> Latent Len: {latent_len}, Dict Size: {num_embeddings}")
    ...
    total_epochs = 100
    # Ustawiamy moment, w którym harmonogram osiąga swój ostateczny limit (np. 80. epoka)
    ...
    for epoch in range(total_epochs):
        ...
        # Alpha: Spada z 20.0 do 10.0 (Agent nadal dba o cel)
        alpha_mse = 20.0 - (10.0 * progress)     
        
        # DRASTYCZNA PODWYŻKA KARY ZA MUTACJE (Beta)
        # Zaczynamy od 0.5 (kara -90 za 180 mutacji) 
        # Rosniemy do 2.5 (kara -450 za zniszczenie całej nici)
        ...
        for batch_idx, (X_batch, Y_dev, Y_hk) in enumerate(train_loader):
            ...
            # ========================================================
            # FAZA 1: VQ-VAE Fine-Tuning + Grupowanie Słownika
            # ========================================================
            vqvae.train()
            opt_vq.zero_grad()
            ...
            # ========================================================
            # FAZA 2: REINFORCEMENT LEARNING (Aktor eksploruje)
            # ========================================================
            navigator.train()
            vqvae.eval() 
            opt_rl.zero_grad()
            ...
            # ========================================================
            # FAZA 3: ŚRODOWISKO OCENIA AGENTA (Reward)
            # ========================================================
            with torch.no_grad():
                ...
                # Używamy zaktualizowanych wag alpha i beta!
                reward = - (alpha_mse * mse_total) - (beta_mut * mutations_count)

            # ========================================================
            # FAZA 4: AKTUALIZACJA AGENTA
            # ========================================================
