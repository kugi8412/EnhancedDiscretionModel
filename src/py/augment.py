...
class NormalizeNoise(RandomNoise):
    """
    Randomly adds Gaussian noise to a batch of sequences and applies a Softmax 
    function along the nucleotide channel dimension.

    Unlike standard RandomNoise, which can produce mathematically invalid states 
    for one-hot encoded data (e.g., negative values or sums greater than 1), 
    this augmentation ensures the output remains a valid probability distribution. 
    The values across the alphabet (A, C, T, G) for each position will strictly 
    sum to 1.0. This biologically simulates sequencing uncertainty or continuous 
    position probability matrices (PPM).

    Parameters
    ----------
    noise_mean : float, optional
        Mean of the Gaussian noise before Softmax application, defaults to 0.0.
    noise_std : float, optional
        Standard deviation of the Gaussian noise before Softmax application, defaults to 0.2.
    """

    def __call__(self, x):
        """
        Randomly adds Gaussian noise and normalizes a set of one-hot DNA sequences.
        ...
        -------
        torch.Tensor
            Sequences with normalized random noise, where values along dimension 1
            (nucleotide channels) are strictly positive and sum to 1.0.
        """
        noise = torch.normal(self.noise_mean, self.noise_std, x.shape).to(x.device)
        x_noisy = torch.nn.functional.softmax(x + noise, dim=1) 
