...
def predicted_vs_observed(true, predicted, title, class_names=['dev', 'hk'], save_path=None):
    ...
    if save_path:
        ...
        print(f"[INFO] Wykres zapisano w: {save_path}")
    
    # Zamiast plt.show() używamy plt.close(), by skrypt się nie zawieszał
    plt.close(fig)

def evaluate_model(config_path, weights_path, set_name='Test'):
    """Główna logika ewaluacji z obsługą cVQ-VAE i LegNeta."""
    config = load_config(config_path)
    
    # Wyłączamy augmentacje i szum dla bezstronnej ewaluacji
    if 'data' in config:
    ...
    print(f"[INFO] Ewaluacja modelu {model_name} na zbiorze {set_name}...")
    
    # 1. Ładowanie głównego modelu
    model = build_model(config).to(device)
    model.load_state_dict(torch.load(weights_path, map_location=device))
    model.eval()

    # 2. Ładowanie Wyroczni (Niezbędne dla cVQ-VAE, aby zdobyć predykcję z Gumbela)
    oracle_model = None
    oracle_cfg = config.get('oracle', None)
    if oracle_cfg and oracle_cfg.get('apply', False):
        print(f"[INFO] Wczytywanie zamrożonej Wyroczni do oceny wygenerowanych sekwencji...")
    ...
    recon_acc_list = [] # NOWOŚĆ: Zbieramy % rekonstrukcji!

    print("[INFO] Generowanie predykcji...")
    with torch.no_grad():
        # Pobieramy X oraz Y!
        for X_batch, Y_dev_batch, Y_hk_batch in dataloader:
            X_batch = X_batch.to(device)
            Y_dev_batch = Y_dev_batch.to(device) # Prawdziwy warunek
            Y_hk_batch = Y_hk_batch.to(device)   # Prawdziwy warunek
            
            try:
                # Podajemy prawdziwe wartości do modułu FiLM, a nie zera!
            ...
            if isinstance(outputs, tuple) and len(outputs) == 3 and oracle_model is not None:
                ...
                # Liczymy rekonstrukcję dla starego VQ
                true_seq_indices = torch.argmax(X_batch, dim=1)
    ...
    print(f"[INFO] Predykcje zapisano w: {pred_filename}")
    ...
    print("\n===========================================")
    print(f" WYNIKI EWALUACJI ({set_name}) - {model_name}")
    print("===========================================")
    print(f" PCC Developmental: {pcc_dev:.4f}")
    print(f" PCC Housekeeping:  {pcc_hk:.4f}")
    if len(recon_acc_list) > 0:
        print(f" Dokładność Rekonstrukcji: {np.mean(recon_acc_list):.2f}%")
    print("===========================================\n")
