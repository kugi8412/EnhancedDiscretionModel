...
try:
    ...
    pass  # Already set by hub app.py
...
def _predict(model, sequences, device, batch_size=256):
    """Run prediction (not cached to avoid cross-model cache collision)."""
    all_dev, all_hk = [], []
    for i in range(0, len(sequences), batch_size):
        batch = sequences[i: i + batch_size]
        X = torch.from_numpy(one_hot_encode_dna(batch)).float().to(device)
        with torch.no_grad():
            out = model(X)
        if isinstance(out, (list, tuple)):
            ...
            d, h = out[:, 0:1], out[:, 1:2]
        all_dev.append(d.cpu().squeeze(-1).numpy())
        all_hk.append(h.cpu().squeeze(-1).numpy())
...
if run_btn:
    ...
    for i, (cfg_f, wt_f) in enumerate(zip(cfg_files, wt_files)):
        ...
        with st.spinner(f"Predicting with {tag}…"):
            p_dev, p_hk       = _predict(model, sequences,    device, batch_size=batch_size)
            p_dev_rc, p_hk_rc = (_predict(model, sequences_rc, device, batch_size=batch_size)
